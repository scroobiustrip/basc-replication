...
for (k in 1:ncases) { 
  ...
  result <- foreach(i=1:nsims, .combine=f(),
    ...
    xi <- matrix(0, TT, p)
    for (i in c(1:p)) {
      xi[,i] <- getTS(type = "ar1", TT, ar.coef) * beta[i]
    }
    ...
    data <- simulateCalib(N = N, 
      ...
      ar.coef = ar.coef,
      seed=NULL)
    #panelView(X1 ~ D, data = data, index = index, type = "outcome")
    ...
    tryCatch({
      ...
      T0.match <- floor(seq(1, T0, length.out = (T0-5)))
      Y.list <- vector(mode = "list", length = length(T0.match))
      for (m in 1:length(T0.match)) {
        Y.list[[m]] <- list("Y",T0.match[m],"mean")
      }
    ...
    gsynth.out <-  gsynth(Y ~ D, data = data, index = index, force = "time", 
        CV = 0, r = (r+4), se = TRUE, parallel = FALSE, inference = "parametric")
  ...
  save(storage, file = "./tempdata/sim_single_X.RData")
