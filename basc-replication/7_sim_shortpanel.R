...
N <- 50
m.list <- c(0.8, 0.6, 0.4)
T.list <- c(15, 20, 25, 30) # T0: 5, 10, 15, 20
#N.list <- c(50)
#T.list <- c(50)
cases <- cbind(rep(m.list, each = length(T.list)),
  rep(T.list, length(m.list)))
...
for (k in 1:ncases) { 
  ...
  result <- foreach(i=1:nsims, .combine=f(),
    ...
    data <- simulateSA(N = N, TT = TT, r = r, p = p,
      tr.threshold = m, # e.g. tr.star>=0.5,a D = 1, length(tr.threshold) = ntr - 1
  ...
  save(storage, file = "./tempdata/sim_shortpanel.RData")
